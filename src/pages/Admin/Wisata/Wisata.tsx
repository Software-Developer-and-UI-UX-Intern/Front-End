...
export default function RestoranPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<Restoran[]>('https://tripselbe.fly.dev/wisata');
  ...
  const handleEdit = (nama: string) => {
    navigate(`/admin/input-wisata`, { state: { nama } });
  };
  const handleAdd = () => {
    navigate(`/admin/add-wisata`);
  };

  const handleDelete = async (nama: string) => {
    try {
      await axios.delete(`https://tripselbe.fly.dev/wisata/${nama}`);
  ...
  <Stack sx={{ backgroundColor: '#04214C' }} flexDirection={'column'} margin={'0 20px 0 20px'} width={'calc((372px * 10) + 100px)'} height={'auto'} border={'2px solid #04214C'} borderRadius={'30px 30px 0 0'}>
    ...
    <Stack direction={'row'} sx={{ overflowX: 'none' }}>
      ...
      <Stack minWidth={'372px'} alignItems={'center'} justifyContent={'center'}>
        <Typography fontSize={'26px'} color={'#FFF'} fontWeight={500}>Link Alamat</Typography>
      </Stack>
      <Stack minWidth={'372px'} alignItems={'center'} justifyContent={'center'}>
    ...
    >
      ...
      <Stack minWidth={'370.4px'} alignItems={'center'} justifyContent={'center'} borderRight={'2px solid #04214C'}>
        <Typography fontSize={'26px'} color={'#04214C'} fontWeight={500}>
          {restoran.alamat_url && (restoran.alamat_url.length > 22 ? restoran.alamat_url.slice(0, 22) + '...' : restoran.alamat_url)}
        </Typography>
      </Stack>
      <Stack minWidth={'370.4px'} alignItems={'center'} justifyContent={'center'} borderRight={'2px solid #04214C'}>
        <Typography fontSize={'26px'} color={'#04214C'} fontWeight={500}>
